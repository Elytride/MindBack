...
const MainApp = ({ uploadedFiles, handleFilesUpdate }) => {
  ...
  // Removed the useEffect that clears backend data on load, as it causes data loss.
  // This was identified as a critical bug.
  ...
  if (isLoading) {
      ...
      // Determine if persona selection is needed
      const needsPersonaSelection = Object.keys(activeTabParticipants).length > 0 && Object.keys(activeTab.selectedUserNames).length === 0;
      ...
      if (needsPersonaSelection) {
          mainContentArea = (
              <div className="persona-selection-view">
                  ...
                  {/* The confirm button should only show if there are participants to select */}
                  {Object.keys(activeTabParticipants).length > 0 && (
                      <button
          ...
          // If no persona selection is needed (either participants selected or no participants available),
          // render the ConversationInterface directly.
          mainContentArea = (
              <ConversationInterface
                  ...
                  geminiApiKey={geminiApiKey} // Pass geminiApiKey prop
              />
          );
      }
  } else {
    ...
    <div className={`app-layout ${isSidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
      <header className="app-header">
        ...
        <nav className="app-nav">
          ...
          <button onClick={toggleSettings} className="nav-link settings-button">Settings</button>
        </nav>
      </header>

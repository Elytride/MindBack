...
generation_config = genai.GenerationConfig(
    temperature=chatbot.API_TEMPERATURE
)
...
def start_chat_session():
    data = request.get_json()
    if not data or 'year' not in data or 'apiKey' not in data:
        return jsonify({'error': 'Invalid request data. Requires year and apiKey.'}), 400
    ...
    try:
        ...
        print("Gemini API configured with key from frontend.")
...
"""
        ...
        if chat_session is None:
             return jsonify({'error': f'Failed to start chat session for year {year}. Check backend logs for details.'}), 500
...
def remove_api_key_from_env():
    env_path = os.path.join(os.path.dirname(__file__), '.env')
    if os.path.exists(env_path):
        print(f"Removing GEMINI_API_KEY from {env_path}...")
        try:
            ...
            with open(env_path, 'w') as f:
                for line in lines:
                    if not line.strip().startswith('GEMINI_API_KEY='):
                        f.write(line)
            print("GEMINI_API_KEY removed.")
        except Exception as e:
            print(f"Error removing GEMINI_API_KEY from .env: {e}")
